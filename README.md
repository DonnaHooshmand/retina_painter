...
- **Foreground-aware corrective sampling** — The raw dataset does not need advance labels or artificial class balancing. As corrections accumulate, training draws half its tiles from red-bearing supervision and half from green-bearing supervision whenever both exist, with automatic fallback during cold start. The selected correction must fall inside the model's supervised output region, and seeded trials retain deterministic sampling order.
