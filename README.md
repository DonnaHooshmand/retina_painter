...
**UI checkpoint selection, rollback, and early stopping:** before validation contains foreground, same-project predictions use live candidate weights and are explicitly logged as provisional so early learning is not hidden behind a background-only checkpoint. Once foreground validation exists, predictions use the latest durable validation-best checkpoint rather than transient candidate weights. The candidate is promoted when continuous masked Dice + 0.3 CE improves and is automatically restored to the UI checkpoint after three consecutive materially worse validation epochs. An annotation change grants one learning epoch before regression counting resumes. Foreground-free validation may provisionally promote checkpoints using background CE, but it is not treated as lesion-informed evidence. The default early-stopping patience is 60 epochs; raise it for hard, slow-to-converge biomarkers with `--max-epochs-without-progress N` (e.g. `--max-epochs-without-progress 120`). Hard pixel F1 is diagnostic only. These are internal training mechanisms, not the clinical endpoint. Clinical model comparison uses patient-separated B-scan RIPL detection.
