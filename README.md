...
- **Adaptive corrective sampling** — The raw dataset does not need advance labels or artificial class balancing. As corrections accumulate, the probability of drawing from the red-bearing pool adapts to `foreground_pool / (foreground_pool + background_pool)`. This gives each correction-pool member approximately equal expected sampling frequency instead of repeatedly oversampling a small positive pool. The percentage is a crop-routing probability, not an estimate of RIPL prevalence; an annotation containing both red and green corrections belongs to both pools. The selected correction must fall inside the model's supervised output region, sampling falls back automatically when either pool is empty, and seeded trials retain deterministic sampling order.
...
- **Rare-lesion checkpoint control** — During foreground-free warm-up, same-project predictions use explicitly provisional live weights only while their background validation loss remains within a fixed safety margin of the retained checkpoint; a false-positive-heavy candidate continues training in memory but is withheld from the painter. Once validation contains foreground, the painter uses a stable validation-best UI checkpoint while a candidate continues training. The candidate is promoted using continuous masked combined Dice + 0.3 CE and automatically rolled back after sustained regression; hard pixel F1 remains diagnostic.
...
**UI checkpoint selection, rollback, and early stopping:** before validation contains foreground, same-project predictions use live candidate weights only while their background-only validation loss is no more than `0.01` above the retained checkpoint. A candidate outside that safety margin continues training but painter inference temporarily uses the durable provisional checkpoint. Background-only validation may provisionally promote checkpoints using background CE, but it cannot trigger candidate rollback because it cannot assess RIPL sensitivity. Once foreground validation exists, predictions use the latest durable validation-best checkpoint rather than transient candidate weights. The candidate is promoted when continuous masked Dice + 0.3 CE improves and is automatically restored to the UI checkpoint after three consecutive materially worse validation epochs. Annotation updates reset early stopping and restart the three-epoch rollback grace period so the candidate has time to learn the expanded correction set; both models are then re-evaluated on the same current validation set. The default early-stopping patience is 60 epochs; raise it for hard, slow-to-converge biomarkers with `--max-epochs-without-progress N` (e.g. `--max-epochs-without-progress 120`). Hard pixel F1 is diagnostic only. These are internal training mechanisms, not the clinical endpoint. Clinical model comparison uses patient-separated B-scan RIPL detection.

Engineering trial records and prospective dataset construction are documented in [docs/unet_ripl_trials_2026.md](docs/unet_ripl_trials_2026.md) and [docs/ripl_dataset_2026.md](docs/ripl_dataset_2026.md).
...
**Adaptive sampler terminal message:** During training, the trainer reports `foreground tile target XX% (adaptive)`. `XX%` changes as red- and green-bearing annotations are added; it is not expected to remain at a particular value. For example, pools containing 7 foreground-bearing and 25 background-bearing annotations produce `7 / (7 + 25) = 21.875%`, displayed as 22%. The important operational check is the `(adaptive)` label, not the exact percentage. A fixed percentage is retained only as an explicit developer-side ablation.
