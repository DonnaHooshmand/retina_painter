...
- **Rare-lesion checkpoint control** — During foreground-free warm-up, same-project predictions use explicitly provisional live weights so early learning remains visible. Once validation contains foreground, the painter uses a stable validation-best UI checkpoint while a candidate continues training. The candidate is promoted using continuous masked combined Dice + 0.3 CE and automatically rolled back after sustained regression; hard pixel F1 remains diagnostic.
