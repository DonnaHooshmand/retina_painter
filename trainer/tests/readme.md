...
| `test_training_control.py` | 19 | Trial seeding and RNG isolation, continuous-loss checkpoint promotion below the hard-F1 threshold, background-only validation, deterministic foreground/background corrective sampling and fallback, U-Net context-border supervision, provisional live warm-up, stable UI checkpoint routing, foreground-gated automatic rollback, and regression-counter preservation across annotation updates. |
