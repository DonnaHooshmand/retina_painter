...
**Training sampler:** `TrainDataset` retains RootPainter's inherited minimum of 612 randomly generated crops per epoch. It builds pools from the sparse corrections already collected. By default, the foreground tile target adapts to `foreground_pool / (foreground_pool + background_pool)`, which gives each pool member equal expected sampling frequency without assuming a balanced image collection. This fraction is a crop-routing probability, not an estimate of image- or patient-level RIPL prevalence; a file containing both red and green corrections is present in both pools. The trainer reports `foreground tile target XX% (adaptive)`, where `XX` is expected to change as annotations accumulate. For example, 7 foreground-bearing and 25 background-bearing pool entries produce a 21.875% target, displayed as 22%; the operational check is `(adaptive)`, not a fixed expected percentage. A fixed fraction remains available for controlled ablations, and sampling falls back automatically when either pool is empty. Each crop must contain its selected correction type in the model's actual output region; for U-Net this also prevents an annotation in the discarded 36-pixel context border from admitting a zero-supervision crop. Pool filenames are sorted and all random choices remain governed by the trial seed.
...
- **Painter feedback uses a stable UI checkpoint after foreground-informed validation begins.** During foreground-free warm-up, same-project predictions use explicitly provisional live candidate weights only while background validation loss remains within `0.01` of the retained checkpoint. A candidate beyond that false-positive safety margin continues training in memory while the painter temporarily uses the durable checkpoint. Background CE may provisionally promote checkpoints but cannot trigger rollback because it cannot assess sensitivity. After foreground appears in validation, the in-memory candidate is promoted only when the continuous validation objective improves. Three consecutive materially worse validation epochs automatically restore the UI checkpoint with a fresh optimizer, so a doctor does not have to Stop/Start training manually. Annotation changes reset early stopping but preserve the candidate regression counter because both models are re-evaluated on the same current validation set.
...
python -m pytest test_loss.py test_unet.py test_metrics.py test_loss_masking.py \
                  ...
                  test_instructions.py test_training_control.py \
                  test_nested_annotation_datasets.py -v
...
A review pass over the model/loss/training code produced these fixes, covered by the model/loss/metrics unit suites:
...
- `trainer.py`: `retfound_rfa` shares `in_w=out_w=224`; uses the common combined-loss default plus AdamW (lr=1e-4), and calls `freeze_encoder_blocks(21)` after first model load. Tversky remains an explicit ablation.
...
**Phase 1 status — DONE (2026-04-27).** Untouched pixels were previously leaking into the loss: `outputs[:, c] *= defined_tiles` zeroed the logits, but `softmax(0,0) = (0.5, 0.5)` so each undefined pixel still incurred a constant `~log(2)` CE penalty (and a Dice/Tversky contribution). The fix:

- `loss.py`: `combined_loss(predictions, labels, mask=None)` and `tversky_loss(..., mask=None)` now apply the mask **inside** the loss, so untouched pixels truly contribute zero. `mask=None` preserves legacy behavior for any caller that doesn't pass a mask.
- `trainer.py` (and `model_utils.epoch`): pass `defined_tiles` to the loss; remove the old `outputs[:, c] *= defined_tiles` trick.
- `datasets.py`: `mask = np.logical_or(foreground, background)` plus an explicit assertion that fg/bg never overlap (defensive against malformed PNGs and any future code path that merges the model prediction into the annotation file).
- `model_utils.unet_segment`: added a defensive `cnn = cnn.to(device)` so callers that wrap a model in `DataParallel` without moving it (e.g. tests) don't trip an MPS / CUDA vs CPU mismatch on RETFound's registered ImageNet-normalization buffers. Production callers (`load_model`, `ensemble_segment`) already moved the model — this is just a guardrail.
- Regression tests in `trainer/tests/test_loss_masking.py` (18 cases) lock in the contract: zero gradient and zero loss-value sensitivity in untouched regions, invariance to untouched-canvas size, parity with legacy unmasked loss when mask is all-1s, correct model-to-loss routing, and explicit ablation overrides. End-to-end UNet smoke at `smoke_phase1.py` and RETFound + RETFound-RFA GPU smoke at `smoke_phase1_gpu.py`.

**Expected loss-value shift after the fix:** average training loss drops in proportion to the untouched fraction of each tile (the constant CE penalty per untouched pixel goes away). On a 50%-untouched synthetic tile measured with the smoke test, `combined_loss` drops by ~0.14 nats and `tversky_loss` by ~0.06 nats; at 75% untouched the drops are ~0.25 and ~0.13 respectively. The shape is roughly linear in untouched fraction. This is **not** a regression — it's the leak being removed.
